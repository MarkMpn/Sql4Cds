...
namespace MarkMpn.Sql4Cds.Export.DataStorage
{
    ...
    public class ServiceBufferFileStreamWriter : IFileStreamWriter
    {
        ...
        public ServiceBufferFileStreamWriter(Stream stream)
        {
                ...
                if (reader.HasLongColumns)
                {
                    ...
                    else
                    {
                        if (ci.IsLong.HasValue && ci.IsLong.Value)
                        {
                            ...
                            if (ci.IsBytes)
                            {
                                values[i] = reader.GetBytesWithMaxCapacity(i, Int32.MaxValue);
                            }
                            else if (ci.IsChars)
                            {
                                values[i] = reader.GetCharsWithMaxCapacity(i, Int32.MaxValue);
                            }
                            else if (ci.IsXml)
                            {
                                values[i] = reader.GetXmlWithMaxCapacity(i, Int32.MaxValue);
                            }
                            else
                            {
