...
namespace MarkMpn.Sql4Cds.Engine.ExecutionPlan
{
    ...
    abstract class BaseDataNode : BaseNode, IDataExecutionPlanNodeInternal
    {
        ...
        /// </summary>
        /// <param name="primaryDataSource">The main <see cref="DataSource"/> for this connection</param>
        /// <param name="metadata">The <see cref="IAttributeMetadataCache"/> to use to get metadata</param>
        /// <param name="options"><see cref="IQueryExpressionVisitor"/> to indicate how the query can be executed</param>
        ...
        protected bool TranslateFetchXMLCriteria(DataSource primaryDataSource, IAttributeMetadataCache metadata, IQueryExecutionOptions options, BooleanExpression criteria, INodeSchema schema, string allowedPrefix, string targetEntityName, string targetEntityAlias, object[] items, IDictionary<string, DataTypeReference> parameterTypes, out filter filter)
        {
            if (!TranslateFetchXMLCriteria(primaryDataSource, metadata, options, criteria, schema, allowedPrefix, targetEntityName, targetEntityAlias, items, parameterTypes, out var condition, out filter))
                return false;
        ...
        /// <returns><c>true</c> if the <paramref name="criteria"/> can be translated to FetchXML, or <c>false</c> otherwise</returns>
        private bool TranslateFetchXMLCriteria(DataSource primaryDataSource, IAttributeMetadataCache metadata, IQueryExecutionOptions options, BooleanExpression criteria, INodeSchema schema, string allowedPrefix, string targetEntityName, string targetEntityAlias, object[] items, IDictionary<string, DataTypeReference> parameterTypes, out condition condition, out filter filter)
        {
            condition = null;
            filter = null;
            ...
            {
                if (!TranslateFetchXMLCriteria(primaryDataSource, metadata, options, binary.FirstExpression, schema, allowedPrefix, targetEntityName, targetEntityAlias, items, parameterTypes, out var lhsCondition, out var lhsFilter))
                    return false;
                if (!TranslateFetchXMLCriteria(primaryDataSource, metadata, options, binary.SecondExpression, schema, allowedPrefix, targetEntityName, targetEntityAlias, items, parameterTypes, out var rhsCondition, out var rhsFilter))
                    return false;
            ...
            {
                return TranslateFetchXMLCriteria(primaryDataSource, metadata, options, paren.Expression, schema, allowedPrefix, targetEntityName, targetEntityAlias, items, parameterTypes, out condition, out filter);
            }
            ...
            if (criteria is BooleanComparisonExpression comparison)
            {
                ...
                if (field == null || (literal == null && func == null && variable == null && parameterless == null && globalVariable == null && (field2 == null || !options.ColumnComparisonAvailable) && !expr.IsConstantValueExpression(primaryDataSource, schema, options, out literal)))
                    return false;
                ...
                {
                    if (func.IsConstantValueExpression(primaryDataSource, schema, options, out literal))
                        values = new[] { literal };
                    else
                        return false;
                }
                ...
                {
                    if (parameterless.IsConstantValueExpression(primaryDataSource, schema, options, out literal))
                    {
                        values = new[] { literal };
                    }
