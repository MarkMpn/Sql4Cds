    ...
    abstract class BaseDataNode : BaseNode, IDataExecutionPlanNode
    {
        ...
        private bool TranslateFetchXMLCriteria(IAttributeMetadataCache metadata, IQueryExecutionOptions options, BooleanExpression criteria, NodeSchema schema, string allowedPrefix, string targetEntityName, string targetEntityAlias, object[] items, out condition condition, out filter filter, IDictionary<object,List<FetchLinkEntityType>> additionalLinkEntities)
        {
            ...
            if (criteria is BooleanComparisonExpression comparison)
            {
                ...
                if (field2 == null)
                {
                    return TranslateFetchXMLCriteriaWithVirtualAttributes(meta, entityAlias, attrName, op, values, metadata, targetEntityAlias, items, additionalLinkEntities, out condition, out filter);
                }
                else
                {
            ...
            if (criteria is InPredicate inPred)
            {
                ...
                return TranslateFetchXMLCriteriaWithVirtualAttributes(meta, entityAlias, attrName, inPred.NotDefined ? @operator.notin : @operator.@in, inPred.Values.Cast<Literal>().ToArray(), metadata, targetEntityAlias, items, additionalLinkEntities, out condition, out filter);
            }
            ...
            if (criteria is BooleanIsNullExpression isNull)
            {
                ...
                return TranslateFetchXMLCriteriaWithVirtualAttributes(meta, entityAlias, attrName, isNull.IsNot ? @operator.notnull : @operator.@null, null, metadata, targetEntityAlias, items, additionalLinkEntities, out condition, out filter);
            }
        ...
        private bool TranslateFetchXMLCriteriaWithVirtualAttributes(EntityMetadata meta, string entityAlias, string attrName, @operator op, ValueExpression[] literals, IAttributeMetadataCache metadata, string targetEntityAlias, object[] items, IDictionary<object, List<FetchLinkEntityType>> additionalLinkEntities, out condition condition, out filter filter)
        {
            condition = null;
            filter = null;
