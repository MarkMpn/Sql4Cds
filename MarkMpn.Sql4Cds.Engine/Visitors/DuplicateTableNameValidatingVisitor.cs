        ...
        public override void Visit(NamedTableReference node)
        {
            ...
            if (node.Alias == null)
            {
                if (_tableNames.TryGetValue(node.SchemaObject.BaseIdentifier.Value, out var table))
                    throw new NotSupportedQueryFragmentException($"The objects \"{node.SchemaObject.ToSql()}\" and \"{table.SchemaObject.ToSql()}\" in the FROM clause have the same exposed names. Use correlation names to distinguish them", node);

                if (_tableAliases.TryGetValue(node.SchemaObject.BaseIdentifier.Value, out var alias))
                    throw new NotSupportedQueryFragmentException($"The correlation name '{alias.Alias.Value}' has the same exposed name as table '{node.SchemaObject.ToSql()}'", node);
        ...
        public override void Visit(TableReferenceWithAlias node)
        {
            ...
            if (node.Alias != null)
            {
                if (_tableAliases.ContainsKey(node.Alias.Value))
                    throw new NotSupportedQueryFragmentException($"The correlation name '{node.Alias.Value}' is specified multiple times in a FROM clause", node);

                if (_tableNames.TryGetValue(node.Alias.Value, out var table))
                    throw new NotSupportedQueryFragmentException($"The correlation name '{node.Alias.Value}' has the same exposed name as table '{table.SchemaObject.ToSql()}'", node);
